            ...
            @foreach($ads as $ad)
                ...
                >
                    <div @class(['tp-banner-item tp-banner-height p-relative mb-30 z-index-1 fix', 'tp-banner-item-sm' => $countAds > 2])>
                        ...
                        <div class="tp-banner-content" style="position:absolute; z-index:2; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.3); display:flex; justify-content: center; align-items:center;">
                            <div>
                                @if($subtitle = $ad->getMetaData('subtitle', true))
                                    <span style="color:white;">{!! BaseHelper::clean(nl2br($subtitle)) !!}</span>
                                @endif
                                @if($title = $ad->getMetaData('title', true))
                                    <h3 class="tp-banner-title" style="color:white;">
                                        @if ($ad->url)
                                ...
                                @if($buttonLabel = $ad->getMetaData('button_label', true))
                                    <div class="tp-banner-btn">
                                        @if ($ad->url)
                                            <a href="{{ $ad->click_url }}" @if($ad->open_in_new_tab) target="_blank" @endif class="tp-link-btn" style="color:white;">
                                        @endif
                                            {{ $buttonLabel }}
